...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
            ...
            for veh in det_vehs:
                if veicoli_cerca.get(veh)=='true' and getEdgeDistrict("districts.taz.xml",veicoli_destinazione.get(veh)) == getParkingDistrict("disParking.xml",parking):
                    if (int)(traci.simulation.getParameter(parking,'parkingArea.capacity')) - (int)(traci.simulation.getParameter(parking,'parkingArea.occupancy')) > 0:
                        ...
                        if veicoli_cerca.get(veh)=='false' and all_edges.get(veicoli_destinazione.get(veh)) == all_parking(parking):
                            seed(step)
                            index = randint(0,2236)
                            while all_edges_keys[index]+'_0' == traci.simulation.getParameter(parking,'parkingArea.lane') or all_edges.get(veicoli_destinazione.get(veh)) != all_edges.get(all_edges_keys[index]):
                                index = randint(0,2236)
        ...
        for edge in all_edges_keys:
            ...
            for veh in det_vehs:
                if veicoli_cerca.get(veh)=='true' and all_edges.get(veicoli_destinazione.get(veh)) == all_edges.get(edge):
                    seed(step)
                    adiacenti = traci.lane.getLinks((str)(edge)+"_0")
                    col = column(adiacenti,0)
                    index = randint(0,len(col) - 1)
                    print("index: "+(str)(index))
                    while col[index] == edge or getEdgeDistrict("districts.taz.xml",veicoli_destinazione.get(veh)) != all_edges.get(col[index]):
                            index = randint(0,len(col) - 1)
                            print("index: "+(str)(index))
                    print("lane: "+(str)(edge)+"_0")
                    traci.vehicle.changeTarget(veh,col[index])
                    print("nuova des per veicolo n."+veh+": "+col[index]+" index: "+(str)(index))
